...
BOOK_KOSHTI_RULES: dict[tuple[int, int], dict[str, str]] = {
    ...
    (0, 2): {"gan": "রাক্ষসগণ", "varna": "বৈশ্যবর্ণ"},   # কৃত্তিকা
...
def _calc_antardasha(
    ...
    if is_first:
        ...
        # 2. Build complete 9-AD timeline using cumulative exact solar days
        full_ads = []
        cur = full_start
        ...
        for i in range(9):
            ...
            # Actual end date via cumulative exact days (final AD closes exactly on md_end)
            if i == 8:
                ad_end = md_end
            else:
